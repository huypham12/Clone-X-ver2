  ...
  .transform((values) => [
    ...new Map(
      values.map((value) => {
        ...
        return [objectId.toHexString(), objectId] as const
      })
    ).values()
  ])
...
const createTweetSchema = z
  .object({
    ...
    mentions: mentionIdsSchema.default([]),
    medias: z
      .array(z.string().refine((val) => ObjectId.isValid(val), { message: 'Invalid media id' }))
      .transform((vals) => vals.map((val) => new ObjectId(val)))
      .default([])
  })
...
const updateTweetSchema = z.object({
  ...
  mentions: mentionIdsSchema.optional(),
  medias: z
    .array(z.string().refine((val) => ObjectId.isValid(val), { message: 'Invalid media id' }))
    .transform((vals) => (vals ? vals.map((val) => new ObjectId(val)) : []))
    .optional()
})
