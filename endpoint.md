...
Rollout notification durable mặc định tắt. Follow cần `NOTIFICATION_OUTBOX_ENABLED=true` và `NOTIFICATION_FOLLOW_OUTBOX_ENABLED=true`; Reply/Quote/Mention cần global flag và `NOTIFICATION_TWEET_OUTBOX_ENABLED=true`; Like/Repost aggregation cần global flag và `NOTIFICATION_SOCIAL_AGGREGATION_ENABLED=true`. Message Reply/Group Mention cần global flag và `NOTIFICATION_MESSAGE_DIRECTED_ENABLED=true`; Message Reaction aggregation cần global flag và `NOTIFICATION_MESSAGE_REACTION_ENABLED=true`; group system/direct notification cần global flag và `NOTIFICATION_GROUP_MANAGEMENT_ENABLED=true`; tweet gốc opt-in cần global flag, TweetCreated outbox và `NOTIFICATION_FOLLOWED_TWEET_ENABLED=true`. Các flag không làm đổi REST request/response. Trước khi bật social aggregation phải chạy dry-run/reconcile retweet duplicate và tạo unique relation index.
...
- `GET /unread-count`: Trả `{ unreadCount, version, updated_at }`. Khi `NOTIFICATION_UNREAD_STATE_ENABLED=true`, dữ liệu lấy từ state đã backfill; trước đó backend fallback về `countDocuments` và trả version `0`.
- `POST /read-all`: Đánh dấu notification chưa đọc/chưa invalidated tới cutoff của request, set `read_at`/`updated_at` và đóng aggregate window nếu có. Item commit sau cutoff vẫn unread. Có thể gọi lặp lại; response `{ updatedCount, unreadCount, version }` và `updatedCount=0` nếu không có transition.
...
Khi message directed flag bật, message reply tạo `message_reply`, group mention tạo `message_mention`, mention thắng reply cho cùng recipient và cả hai dùng `target_type=MESSAGE`. Message thường/thành viên khác chỉ đi qua inbox unread + `@conversation:receive`; backend không đồng thời tạo generic `message` item. Mute conversation không suppress directed in-app item. Đường tạo generic `message` legacy đã bị ngắt khỏi production; khi directed flag tắt backend không tạo activity item cho message thường.
