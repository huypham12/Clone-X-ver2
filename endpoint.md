...
Notification durable và các handler Phase 7–17 mặc định bật cho local. Có thể đặt `false` để rollback/debug: Follow dùng `NOTIFICATION_OUTBOX_ENABLED` + `NOTIFICATION_FOLLOW_OUTBOX_ENABLED`; Reply/Quote/Mention dùng global flag + `NOTIFICATION_TWEET_OUTBOX_ENABLED`; Like/Repost dùng global flag + `NOTIFICATION_SOCIAL_AGGREGATION_ENABLED`; Message Reply/Group Mention dùng global flag + `NOTIFICATION_MESSAGE_DIRECTED_ENABLED`; Message Reaction dùng global flag + `NOTIFICATION_MESSAGE_REACTION_ENABLED`; group system/direct notification dùng global flag + `NOTIFICATION_GROUP_MANAGEMENT_ENABLED`; tweet gốc opt-in dùng global flag + TweetCreated outbox + `NOTIFICATION_FOLLOWED_TWEET_ENABLED`. Các flag không làm đổi REST request/response và không bật lại legacy writer. Nếu dữ liệu local cản unique Retweet index, reset collection `tweets`; không chạy migration/reconcile để giữ dữ liệu cũ.
